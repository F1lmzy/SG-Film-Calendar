...
Works for every source the scrapers emit (Filmhouse.sg and Singapore Film
Society): each film dict supplies ``source``, ``themes``, ``tags``, optional
``poster_url`` / ``synopsis`` / ``language`` / ``country`` metadata, and a list
of ``screenings``. SFS Somerset bundles are already expanded into individual
films with discrete screenings by ``sfs_scraper``, so they archive one row per
film — the same grain as Filmhouse.
"""
...
SFS_DEFAULT_VENUE = "Singapore Film Society"
...
class HistoryStore:
    ...
    def _merge_film(
        ...
        dates |= self._screening_dates(film.get("screenings") or [])
    ...
    def _venue(self, film: Dict) -> str:
        ...
        if venue:
            return venue
        return SFS_DEFAULT_VENUE if film.get("source") == "sfs" else DEFAULT_VENUE

    @staticmethod
    def _screening_dates(screenings: List[Dict]) -> set:
        """Return the set of screening start timestamps as ISO strings.

        Both scrapers emit discrete screenings with a ``start`` datetime —
        Filmhouse one per show, and SFS one per expanded ticket / weekday — so
        each contributes exactly one entry. Multi-day all-day events (with no
        published per-day schedule) are recorded under their start date.
        """
        dates: set = set()
        for screening in screenings:
            start = screening.get("start")
            if start is None:
                continue
            dates.add(
